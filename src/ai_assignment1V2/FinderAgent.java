...
public class FinderAgent<T extends Puzzle> {
    ...
    //GETTERS
    public Node getStart() { return new Node(start);}
    public T getEnd(){ return (T)end.returnDeepCopy();}
    ...
    public LinkedList<Node<T>> search(SearchAlgorithm algo, EnumHeursitic heu){
//        /**
...
//        algo.setLimit(start, new Node<>(end));
        ...
        /**
         * HashMap is use for the closed list, It holds states that have been visited
         * PriorityQueue is used for the open list, It hold all states yet to be visited
         * I went with a PriorityQueue with a comparator as this allows the solution to be
         * highly generic. 
         */
        ...
        /**
         * Set the HEURSITIC_COST for the initial node,
         * and then adds the node to the open list.
         */
        start.setHEURSITIC_COST(heu.value(start.getData(), end));
        open.add(start);  
        
        /**
         * The main loop of searching, with the configurations
         * set out by the algorithm.
         */
        while (!open.isEmpty()){
            /**
             * Grabs the first Item of the queue, the queue is 
             * order by the comparator to fit the algorithm.
             */
            origin = open.poll(); 
            int watcherDepth = algo.getNodeCost(origin);
            /**
             * Checks if the current node has already been visited,
             * if it has been visited then it will skip over the node
             * if it hasn't then it will set the node to be visited.
             */
            ...
            /**
             * Checks the current node to confirm that it
             * complies with the Algorithm.
             */
            ...
            /**
             * Checks if the current states meets the goal state
             * If true then will calculate the explored states
             * and return the path to the goal.
             */
            ...
            /**
             * Will generate the neighbouring nodes,
             * This can either be a node of null.
             */
            Node<T> neighbours[] = origin.genNeighbours();
            
            /**
             * Will loop through the neighbours,
             * and if it isn't null, it will calculate its heursitic
             * add the node to the open list, and then increment
